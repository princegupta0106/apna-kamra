...
function amenityTags(list, max) {
  ...
  if (max && list.length > max)
    html += `<span class="tag accent">+${list.length - max} more</span>`;
  return html;
}
...
function statusBadges(p) {
  let b = "";
  if (p.is_featured)
    b += `<span class="prop-badge featured">⭐ Featured</span>`;
  if (p.is_trusted) b += `<span class="prop-badge trusted">✓ Trusted</span>`;
  if (p.is_unverified)
    b += `<span class="prop-badge unverified">⚠ Unverified</span>`;
  return b;
}
...
const Visitor = (() => {
  ...
  function askIfNeeded(onDone) {
    ...
    sheet.style.cssText =
      "position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:300;display:flex;align-items:center;justify-content:center;padding:20px;";
    sheet.innerHTML = `
    ...
    document.getElementById("visitorSubmitBtn").onclick = () => {
      ...
      if (val.length !== 10 || !/^[0-9]{10}$/.test(val)) {
        document.getElementById("visitorMsg").textContent =
    ...
    inp.addEventListener("keydown", (e) => {
      if (e.key === "Enter")
        document.getElementById("visitorSubmitBtn").click();
    });
